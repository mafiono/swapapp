...
const fetchTxInfo = async (
  ...
  try {
    const { amount, minerFee, adminFee, size } = isMultisign
      ? await actions.btcmultisig.fetchTxInfo(txHash, 8000, serviceFee)
...
describe('BTC Send Tests', () => {
  ...
  it('send and check transaction via regular wallet with 1000 satoshis', async () => {
    ...
    const { amount, minerFee, size } = await fetchTxInfo(txHash)
    expect(amount).toBe(options.amount)
    expect(minerFee).toBeWithinRange(
      toFloorValue(options.feeValue),
      toCeilingValue(options.feeValue)
    )
    expect(size).toBeWithinRange(
      toFloorValue(new BigNumber(txSize)),
      toCeilingValue(new BigNumber(txSize))
    )
  }, 145000)
  ...
  it('send and check transaction via regular wallet with 1000 satoshis with adminFee', async () => {
    ...
    let unspents = await actions.btc.fetchUnspents(options.from)
    unspents = await actions.btc.prepareUnspents({
      amount: feeFromAmount.plus(options.amount).plus(options.feeValue).dp(8).toNumber(),
      unspents,
    })
    const txIn = unspents.length
    const txOut = 3
    ...
    const txHash = await actions.btc.send(options)
    await timeOut(5 * 1000)
    const { amount, minerFee, adminFee, size } = await fetchTxInfo(txHash, false, serviceFee)
  ...
  it('send and check transaction via pin-protected wallet sign with mnemonic with 1000 satoshis', async () => {
    await actions.btcmultisig.login_PIN('cNS7asAySqoMuxZZ37kdgpcEAbgyyEHzuLPHSi9ZLSpFvjcrMJ2T', [
    ...
    const options = {
      ...
      password: '1234',
      mnemonic:
        'execute tunnel height sponsor raccoon random federal infant reform foil fall physical',
    }
    ...
    await timeOut(5 * 1000)
    const { amount, minerFee, size } = await fetchTxInfo(txHash.txId, true)
    expect(amount).toBe(options.amount)
    expect(minerFee).toBeWithinRange(
      toFloorValue(options.feeValue),
      toCeilingValue(options.feeValue)
  ...
  it('send and check transaction via pin-protected wallet sign with password with 1000 satoshis with adminFee', async () => {
    ...
    await actions.btcmultisig.login_PIN('cRQL8PDx7WRJzdi6g3fAagLA3bEc4XMfjDDkDkuheFcX5TRHqEMX', [
      '02094916ddab5abf215a49422a71be54ceb92c3d8114909048fc45ee90acdb5b32',
    ...
    const txHash = await actions.btcmultisig.sendPinProtected(options)
    await timeOut(5 * 1000)
    const { amount, minerFee, adminFee, size } = await fetchTxInfo(txHash.txId, true, serviceFee)
    expect(amount).toBe(options.amount)
    expect(adminFee).toBe(feeFromAmount.toNumber())
    expect(minerFee).toBeWithinRange(
      toFloorValue(options.feeValue),
