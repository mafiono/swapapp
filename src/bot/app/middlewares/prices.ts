    ...
    .then((json: any) => {
      if (json && json.data) {
        ...
        if (base.toUpperCase() === `BTC`) {
          btcData = json.data.filter(
            (item) => item && item.symbol && item.symbol.toUpperCase() === `BTC`
          )
          if (btcData.length) {
            btcData = btcData[0]
          }
        ...
        const btcPrice =
          btcData && btcData.quote && btcData.quote.USD && btcData.quote.USD.price
            ? btcData.quote.USD.price
            : null

        let symbolInfo = json.data.filter(
          (item) => item && item.symbol && item.symbol.toUpperCase() === symbol.toUpperCase()
        )
