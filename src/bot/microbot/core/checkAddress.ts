...
import { TRADE_CONFIG, TRADE_LIMITS } from '../../config/constants'
...
const checkSWAP = (options) => {
  ...
  const amountWithTheSmallestSpread = TRADE_CONFIG['SWAP-BTC'].orders.sort(
    (prev, next) => prev.spreadBuy > next.spreadBuy
  )[0].amount
  ...
  return request(`${CHECK_SWAP_API}?addr=${participantAddress}`)
    ...
    .then((data) => {
      ...
      if (isBuyAmountGreaterThanLimit) {
        debug(
          'checkSWAP:',
          `buy amount is greater than limit. Limit: ${tradeLimit} Buy amount: ${buyAmount}`
        )
      }
