...
export default (app, wallet, orders) =>
  async ({ orderId, participant }) => {
    ...
    if (!order) {
      return handleError(
        new Error(`not found such order, maybe creator is offline? id: ${orderId}`)
      )
    }
    ...
    if (!isAccepted) {
      ...
      return handleError(
        new Error(`is enough balance: ${isEnoughBalance},
