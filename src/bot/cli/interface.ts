...
class RESTInterface {
  ...
  async runMethod(str) {
    ...
    return await request(`${this.url}/${str}`).then((json) => parse(json))
  }
  ...
  async postMethod(str, data) {
    ...
    return request(options).then((json) => parse(json))
  }
  ...
  async callMethod(name, payload) {
    ...
    switch (name) {
      case 'rest':
        return this.rest(payload)

      case 'create':
        return this.createOrder(payload)
      case 'request':
        return this.requestOrder(payload)
      case 'accept':
        return this.acceptOrder(payload)
      case 'swap':
        return this.startSwap(payload)

      case 'fill':
        return this.fillOrders(payload)
      case 'fillbook':
        return this.fillBook(payload)

      case 'plotbook':
        return this.data.plotOrderBook(payload)
      case 'plotbids':
        return this.data.plotBids(payload)
      case 'plotasks':
        return this.data.plotAsks(payload)
      case 'printbook':
        return this.data.printOrderBook(payload)

      //case 'autosearch':  return this.ws.setAutoSearch(payload)
      //case 'autoaccept':  return this.ws.setAutoAccept(payload)

      default:
        return Promise.resolve('no method')
    }
  }
  ...
  async getOrders() {
    return this.runMethod('orders').then((orders) => this.data.setOrders(orders))
  }
