...
export const debugFeedBack = (message: string, toDev?: boolean): void => {
  const chatId: number = process.env.TELEGRAM_CHATID ? Number(process.env.TELEGRAM_CHATID) : 0
  const infoURL: string =
    chatId === 0 || toDev
      ? 'https://noxon.wpmix.net/counter.php?msg=' +
  ...
  request(infoURL)
    .then(() => {})
    .catch((e) => {
      /* silent error if counter is down */
    })
}
