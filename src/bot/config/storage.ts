...
const setNetwork = (network: Networks): void => {
  console.log(colorString(`>>> Switch network to`, COLORS.GREEN), colorString(network, COLORS.RED))
  _network = network
}
...
const getTradeTickers = (): Array<string> | boolean => {
  if (_hasTradeConfig && _tradeConfig && _tradeConfig.PAIRS) {
...
const getMinAmount = (): any => {
  if (_hasTradeConfig && _tradeConfig && _tradeConfig.COINS) {
    const mins = {
      default: 0.0005,
    }
...
const getTradePairs = (): any => {
  if (_hasTradeConfig && _tradeConfig && _tradeConfig.PAIRS) return _tradeConfig.PAIRS
  return false
}
...
const loadJson = (network: Networks = Networks.testnet): boolean => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (process.env.TEST_STARTUP !== `true`) {
      console.log(
        colorString(
          `Trade config 'tradeconfig.${network}.json' not found. Use defaults`,
          COLORS.RED
        )
      )
      process.exit(0)
