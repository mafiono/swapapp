...
export default class BtcToEthLikeToken extends AtomicAB2UTXO {
  ...
  constructor(swap: Swap, options: IBtcToEthLikeToken) {
    ...
    if (!options.blockchainName) {
      throw new Error(
        `BtcToEthLikeToken - token ${options.tokenName} - option blockchainName requery`
      )
    }
    if (!options.getMyAddress || typeof options.getMyAddress !== 'function') {
      throw new Error(
        `BtcToEthLikeToken ${options.blockchainName} - token ${options.tokenName} - option getMyAddress - function requery`
      )
    }
    if (!options.getParticipantAddress || typeof options.getParticipantAddress !== 'function') {
      throw new Error(
        `BtcToEthLikeToken ${options.blockchainName} - token ${options.tokenName} - option getParticipantAddress - function requery`
      )
    }
    ...
    if (this.isMaker()) {
      this.swap.room.once('create eth contract', async ({ ethSwapCreationTransactionHash }) => {
        flow.setState(
          {
          ...
          await util.helpers.repeatAsyncUntilResult(async () => {
            ...
            try {
              ...
              if (utxoWithdrawData) {
                ...
                const secret = await this.btcSwap.getSecretFromTxhash(
                  utxoSwapWithdrawTransactionHash
                )
                if (secret) {
                  this.finishStep(
