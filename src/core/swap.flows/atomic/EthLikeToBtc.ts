...
class EthLikeToBtc extends AtomicAB2UTXO {
  ...
  constructor(swap, options: IEthLikeToBtc) {
    ...
    if (!options.getParticipantAddress || typeof options.getParticipantAddress !== 'function') {
      throw new Error(
        `EthLikeToBtc ${options.flowName} - option getParticipantAddress - function requery`
      )
    }
    ...
    if (this.isTaker()) {
      ...
      flow.swap.room.on('ethWithdrawTxHash', (data) => {
        const { ethLikeSwapWithdrawTransactionHash } = data
        flow.setState({
  ...
  _getSteps() {
    ...
    if (this.isMaker()) {
      ...
      return [
        ...
        async () => {
          this.waitUTXOScriptFunded().then((funded) => {
            if (funded) {
