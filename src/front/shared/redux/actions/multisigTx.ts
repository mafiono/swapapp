...
const fetchTx = (txId) => apiLooper.get('multisig', `/tx/${txId}`).then((txData) => txData)
  ...
  return apiLooper
    ...
    .then((res: any) => {
      if (res && res.answer) {
        ...
        const pendingTransactions: any[] = res.items.filter(
...
const goToLastWallet = () => {
  ...
  const walletsWithRequests = Object.keys(multisigStatus)
    .map((walletAddress) => ({
      walletAddress,
      ...multisigStatus[walletAddress],
    }))
    .filter((item) => {
      const { count, pending } = item
