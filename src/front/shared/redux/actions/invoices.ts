...
const cancelInvoice = (invoiceId) =>
  new Promise((resolve) =>
    apiLooper
      .post('invoiceApi', `/invoice/cancel/`, {
        body: {
          invoiceId,
        },
      })
...
const markInvoice = (invoiceId, mark, txid, address) =>
  new Promise((resolve) =>
    apiLooper
      .post('invoiceApi', `/invoice/mark/`, {
        body: {
          invoiceId,
          mark,
          txid,
  ...
  return new Promise((resolve) => {
    ...
    const wallets = data.map((item) => {
      if (item && item.type && item.address) {
        const { type: rawType, tokenKey, address } = item
    ...
    apiLooper
      .post('invoiceApi', `/invoice/fetchmany/`, {
        body: {
          wallets,
          mainnet: process.env.MAINNET ? '1' : '0',
        },
