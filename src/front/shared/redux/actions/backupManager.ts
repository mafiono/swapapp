...
const serverCleanupSeed = () => {
  if (config && config.opts && config.opts.plugins && config.opts.plugins.backupPlugin) {
...
const backup = (mark, label, overide) => {
  ...
  localStorage.setItem(
    `${constants.localStorage.backups_prefix}:${mark}`,
...
const importBackup = (backup) => {
  if (backup && backup.mark && backup.utx && backup.label && backup.data) {
