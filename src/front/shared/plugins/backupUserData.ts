  ...
  hasServerBackup: () => {
    return new Promise((resolve) => {
      ...
      if (
        ...
        axios
          .post(plugins.restorePluginUrl, {
            WPuserUid: window.WPuserUid,
            WPuserHash: config.opts.WPuserHash,
          })
          .then((res: any) => {
            if (res?.data?.answer && res.data.answer === `ok` && res.data.data) {
              resolve(true)
            } else {
              resolve(false)
            }
    ...
    return new Promise<BackupUserResponse>((resolve) => {
      ...
      if (
        ...
        const backup = {
          ...
          didProtectedBtcCreated: localStorage.getItem(
            constants.localStorage.didProtectedBtcCreated
          ),
          didPinBtcCreated: localStorage.getItem(constants.localStorage.didPinBtcCreated),
        }

        axios
          .post(plugins.backupPluginUrl, {
            ...backup,
            WPuserUid: window.WPuserUid,
            WPuserHash: config.opts.WPuserHash,
          })
          .then((res: any) => {
            if (res?.data?.answer && res.data.answer === `ok`) {
